...
// ---------------------------------------------------------------------------
// SAFESCREEN FORK: capture destination changed.
//
// Upstream writes every captured photo to FOLDERID_Pictures — the user's
// Pictures library. For a normal camera app that is the right answer: the user
// asked for a photo and expects to find it. SafeScreen is not a camera app. It
// samples the webcam several times a second purely to decide whether someone is
// looking at the screen, and every one of those frames is discarded microseconds
// later.
//
// Writing them to Pictures means scattering thousands of photographs of the user
// through a folder that is Search-indexed, thumbnailed by Explorer, and on most
// machines synced to OneDrive — i.e. uploaded off the machine. That is the exact
// outcome a privacy tool exists to prevent, and it cannot be fixed from Dart
// because the write happens here, before the path is ever returned.
//
// This fork redirects captures to a private per-user directory under %TEMP%,
// which is not a sync target. SafeScreen still overwrites and deletes each file
// immediately (see lib/services/secure_frame_store.dart); this change is what
// keeps the frames out of cloud storage in the first place.
//
// Video capture is deliberately left pointing at FOLDERID_Videos: SafeScreen
// never records video, and a user-initiated recording genuinely does belong in
// the user's library.
// ---------------------------------------------------------------------------
std::optional<std::string> GetFilePathForPicture() {
  ...
  std::wstring directory = std::wstring(temp_path) + L"SafeScreenFrames";
  if (!CreateDirectoryW(directory.c_str(), nullptr) &&
      GetLastError() != ERROR_ALREADY_EXISTS) {
    return std::nullopt;
  }

  return Utf8FromUtf16(directory) + "\\" + "PhotoCapture_" +
         GetCurrentTimeString() + "." + kPictureCaptureExtension;
}
